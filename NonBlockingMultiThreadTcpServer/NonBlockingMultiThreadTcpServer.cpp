// NonBlockingMultiThreadTcpServer.cpp : 定义控制台应用程序的入口点。
//
...
int main()
{
	...
	while (TRUE)
	{
		...
		sThreadData data;
		memmove(&data.sClient, &sClient, sizeof(sClient));
		memmove(&data.addrClient, &addrClient, sizeof(addrClient));

		DWORD dwThreadId;
		CreateThread(NULL, NULL, AnswerThread, (LPVOID)&data, 0, &dwThreadId);
	}
...
DWORD WINAPI AnswerThread(LPVOID lparam)
{
	...
	sThreadData *data = (sThreadData*)(LPVOID)lparam;
	SOCKET sClient = (*data).sClient;
	SOCKADDR_IN addrClient = (*data).addrClient;
